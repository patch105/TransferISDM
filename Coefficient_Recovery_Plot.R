...
b1 <- ggplot(extrap.scenario.df, aes(x = extrap.type, y = beta1, fill = mod.type)) +
  geom_boxplot() +
  geom_hline(yintercept = beta1, linetype = "dashed", color = "red") +
  geom_errorbar(aes(ymin = beta1_25, ymax = beta1_975), width = 0.2, position = position_dodge(0.9)) +
...
b2 <- ggplot(extrap.scenario.df, aes(x = extrap.type, y = beta2, fill = mod.type)) +
  geom_boxplot() +
