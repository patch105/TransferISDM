...
# Now the intensity (not logged)
true_int <- exp(mu)

true_int.rast <- rast(true_int)

crs(true_int.rast) <- crs(cov1) 

names(true_int.rast) <- "int"

true_int.rast <- true_int.rast + cellSize(cov1)
...
<!-- # Trim to just xy -->
<!-- po.rand.gridA.no.bias <- po.rand.gridA[, c("x", "y")] -->
...
Model$m.int.GRF <- isdm(observationList = list(POdat = PO,
                                 ...
                                 biasFormula = ~1 + bias, #Intercept only
...
# Predict from fitted to Projection Site (INTENSITY)
...
cov.rep <- covs.SiteB.rast

# Add a dummy bias variable to keep RISDM happy
cov.rep$bias <- 1 

Model <- imap(Model, function(x, name) {
  
  if (grepl("PO", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
    
    x$preds.PO.int.GRF.SiteB <- predict(x,
                                        covars = cov.rep,
                                        S = posterior_nsamps, 
   ...
   if (grepl("PA", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
      
       x$preds.PA.int.GRF.SiteB <- predict(x,
                                        covars = cov.rep,
                                        S = posterior_nsamps, 
...
# Predict from fitted to Projection Site (LOG INTENSITY)
...
# Pull out posterior variance of the GRF to add to prediction

```{r}

GRF_sd_df <- data.frame(model = c("m.PO.GRF", "m.PA.GRF", "m.int.GRF"),
                   GRF_sd = c(summary(Model$m.PO.GRF)$SPATIAL$mean[2], 
                            summary(Model$m.PA.GRF)$SPATIAL$mean[2],
                            summary(Model$m.int.GRF)$SPATIAL$mean[2])) %>% 
  mutate(GRF_var = GRF_sd^2) 

GRF_sd_df <- GRF_sd_df %>% 
  mutate(variance.term = exp(0.5*GRF_var),
         variance.term.log.int = 0.5*GRF_var)


```

# Add to the prediction at the projection site (INTENSITY)
Pull out the Median predictions and then multiply by the variance term. 
...
Model <- imap(Model, function(x, name) {
  ...
  if (grepl("PO", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
    ...
    variance.term <- GRF_sd_df$variance.term[GRF_sd_df$model == name]
    
    x$MEDIAN_preds.PO.int.GRF.SiteB <- x$preds.PO.int.GRF.SiteB$field$Median
    
    x$MEDIAN_preds.PO.int.GRF.SiteB_UPDATE <- x$MEDIAN_preds.PO.int.GRF.SiteB*variance.term
   ...
   if (grepl("PA", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
     ...
     x$MEDIAN_preds.PA.int.GRF.SiteB <- x$preds.PA.int.GRF.SiteB$field$Median
     
       x$MEDIAN_preds.PA.int.GRF.SiteB_UPDATE <- x$MEDIAN_preds.PA.int.GRF.SiteB*variance.term
    }
...
# Add to the prediction at the projection site (LOG INTENSITY)
Pull out the Median predictions and then multiply by the variance term. 
...
Model <- imap(Model, function(x, name) {
  ...
  if (grepl("PO", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
    ...
    variance.term.log.int <- GRF_sd_df$variance.term.log.int[GRF_sd_df$model == name]
    
    x$MEDIAN_preds.PO.int.GRF.SiteB <- x$preds.PO.int.GRF.SiteB$field$Median
    
    x$MEDIAN_preds.PO.int.GRF.SiteB_UPDATE <- x$MEDIAN_preds.PO.int.GRF.SiteB+variance.term.log.int
   ...
   if (grepl("PA", name, fixed = TRUE) | grepl("int", name, fixed = TRUE)) {
     ...
     x$MEDIAN_preds.PA.int.GRF.SiteB <- x$preds.PA.int.GRF.SiteB$field$Median
       ...
       x$MEDIAN_preds.PA.int.GRF.SiteB_UPDATE <- x$MEDIAN_preds.PA.int.GRF.SiteB+variance.term.log.int
    }
...
## Compare with truth (INTENSITY)

```{r}

true_log_int.rast

true_int.rast.SiteB <- crop(true_int.rast, ext(rand.gridB))

range_GRF <- range(values(true_int.rast.SiteB))

truth <- true_int.rast.SiteB %>% 
  as.data.frame(xy = T) %>%  
            ggplot() + 
...
PO <- Model$m.PO.GRF$MEDIAN_preds.PO.int.GRF.SiteB %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("PO model GRF")

PO_update <- Model$m.PO.GRF$MEDIAN_preds.PO.int.GRF.SiteB_UPDATE %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("PO model GRF with variance term")

PA <- Model$m.PA.GRF$MEDIAN_preds.PA.int.GRF.SiteB %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("PA model GRF")

PA_update <- Model$m.PA.GRF$MEDIAN_preds.PA.int.GRF.SiteB_UPDATE %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("PA model GRF with variance term")

int.PO <- Model$m.int.GRF$MEDIAN_preds.PO.int.GRF.SiteB %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("Integrated model PO Intercept GRF")

int.PO_update <- Model$m.int.GRF$MEDIAN_preds.PO.int.GRF.SiteB_UPDATE %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("Integrated model PO Intercept GRF with variance term")

int.PA <- Model$m.int.GRF$MEDIAN_preds.PA.int.GRF.SiteB %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("Integrated model PA Intercept GRF")

int.PA_update <- Model$m.int.GRF$MEDIAN_preds.PA.int.GRF.SiteB %>% 
            as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = Median)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5)) +
            ...
            theme(axis.title.x = element_blank(),
                  ...
                  legend.ticks = element_blank(),
                  legend.title = element_blank()) +
  ggtitle("Integrated model PA Intercept GRF with variance term")

 ggarrange(truth, PO, PO_update, ncol = 3, nrow = 1)
 
 ggarrange(truth, PA, PA_update, ncol = 3, nrow = 1)
  
 ggarrange(truth, int.PO, int.PO_update, ncol = 3, nrow = 1) 
 
 ggarrange(truth, int.PA, int.PA_update, ncol = 3, nrow = 1)

 
 
```

## Compare with truth (LOG INTENSITY)
...
range_GRF <- range(values(true_log_int.rast.SiteB))

truth <- true_log_int.rast.SiteB %>% 
  as.data.frame(xy = T) %>%  
            ggplot() + 
            geom_tile(aes(x = x, y = y, fill = int)) + 
            scale_fill_viridis(guide = guide_colorbar(barwidth = 0.5), limits = range_GRF) +
            coord_fixed() + 
            theme_bw() + 
            theme(axis.title.x = element_blank(),
                  axis.title.y = element_blank(),
